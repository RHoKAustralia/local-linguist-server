begin
  ...
  namespace :metrics do
    ...
    task :mutate, [:path] do |_, args|
      puts args
      system('mutant -r ./config/environment.rb --use ' \
        "rspec #{class_list(args[0]).join(' ')}")
    end
  ...
  puts 'metrics.rake not loaded because gems not available'
end
