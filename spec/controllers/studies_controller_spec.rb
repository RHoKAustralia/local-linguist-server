require 'rails_helper'

RSpec.describe StudiesController, type: :controller do

  let(:valid_attributes) {
    {
      name: "A new study"
    }
  }

  let(:invalid_attributes) {
    { name: nil }
  }

  let(:valid_session) { {} }

  describe "GET #index" do
    it "assigns all studies as @studies" do
      study = Study.create! valid_attributes
      get :index, params: {}
      expect(assigns(:studies)).to eq([study])
    end
  end

  describe "GET #show" do
    it "assigns the requested study as @study" do
      study = Study.create! valid_attributes
      get :show, params: {id: study}
      expect(assigns(:study)).to eq(study)
    end
  end

  describe "GET #new" do
    it "assigns a new study as @study" do
      get :new, params: {}
      expect(assigns(:study)).to be_a_new(Study)
    end
  end

  describe "GET #edit" do
    it "assigns the requested study as @study" do
      study = Study.create! valid_attributes
      get :edit, params: {id: study}
      expect(assigns(:study)).to eq(study)
    end
  end

  describe "POST #create" do
    context "with valid params" do
      it "creates a new Study" do
        expect {
          post :create, params: {study: valid_attributes}
        }.to change(Study, :count).by(1)
      end

      it "assigns a newly created study as @study" do
        post :create, params: {study: valid_attributes}
        expect(assigns(:study)).to be_a(Study)
        expect(assigns(:study)).to be_persisted
      end

      it "redirects to the created study" do
        post :create, params: {study: valid_attributes}
        expect(response).to redirect_to(Study.last)
      end
    end

    context "with invalid params" do
      it "assigns a newly created but unsaved study as @study" do
        post :create, params: {study: invalid_attributes}
        expect(assigns(:study)).to be_a_new(Study)
      end

      it "re-renders the 'new' template" do
        post :create, params: {study: invalid_attributes}
        expect(response).to render_template("new")
      end
    end
  end

  describe "PUT #update" do
    context "with valid params" do
      let(:new_attributes) {
        { name: "Another study" }
      }

      it "updates the requested study" do
        study = Study.create! valid_attributes
        put :update, params: {id: study, study: new_attributes}
        study.reload
        expect(study.name).to eq("Another study")
      end

      it "assigns the requested study as @study" do
        study = Study.create! valid_attributes
        put :update, params: {id: study, study: valid_attributes}
        expect(assigns(:study)).to eq(study)
      end

      it "redirects to the study" do
        study = Study.create! valid_attributes
        put :update, params: {id: study, study: valid_attributes}
        expect(response).to redirect_to(study)
      end
    end

    context "with invalid params" do
      it "assigns the study as @study" do
        study = Study.create! valid_attributes
        put :update, params: {id: study, study: invalid_attributes}
        expect(assigns(:study)).to eq(study)
      end

      it "re-renders the 'edit' template" do
        study = Study.create! valid_attributes
        put :update, params: {id: study, study: invalid_attributes}
        expect(response).to render_template("edit")
      end
    end
  end

  describe "DELETE #destroy" do
    it "destroys the requested study" do
      study = Study.create! valid_attributes
      expect {
        delete :destroy, params: {id: study}
      }.to change(Study, :count).by(-1)
    end

    it "redirects to the studies list" do
      study = Study.create! valid_attributes
      delete :destroy, params: {id: study}
      expect(response).to redirect_to(studies_url)
    end
  end

end
